...
```mermaid
flowchart LR
    A[Start] --> B{ continent is Europe? and 
    language is JavaScript?}
    B -->|Yes| D[store the d to the list]
    D -->E[list++]
    E -->F{ is end of list ?}
    F -->|NO| B
    F -->|Yes| G[return the list]
```
...
2.什么是lambda函数？请举例说明。  
```
定义了一个匿名函数，即只有函数体，没有函数名称，也没有def和return。
...
```
高阶函数是指能够接受一个或多个函数作为参数、或者返回一个函数作为结果的函数。在编程中，高阶函数可以增强代码的灵活性和可重用性，从而提高开发效率。

常见的高阶函数有：map、filter、reduce，它们都是对列表等数据结构进行操作的函数。
```

```python
# 例如： 第一题的编码聚会
# 利用fliter函数，返回满足需求的列表
dev = list(filter(lambda d: d['continent'] =='Europe' and d['language'] == 'JavaScript' ,lst))
```
## 实验总结

本次实验我进一步了解了函数式编程思想，以及学会了如何使用函数式编程来解决一些问题，还了解了高级函数的使用方法，本次实验我收获颇多。
